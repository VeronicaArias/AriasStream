...
#T	      = 0.005 #5.                     # units: 9.8*10e8 yr/h (approx 1Gyr , according to Geraint and Magda 0.98=1).
...
#TestSatName   = 'AndI' #funciona muy bien 
#Param_Vtan_y  =  -250.0

#TestSatName   = 'AndIII' #funciona aparentemente bien 
#Param_Vtan_y  =  -330.0


#TestSatName   = 'AndXI' #funciona bien sin la MW 
#Param_Vtan_y  =  -150.0

#TestSatName   = 'AndXIV' #Funciona Bien!# en un paper http://arxiv.org/pdf/1205.6864v1.pdf dejan a esta por fuera porque asumen que esta en un first infall... 
#Param_Vtan_y  =  -95.0
#
#TestSatName   = 'AndXVI' #? #Funciona mas o menos bien (Bwd) y puede que se solucione con los errores de las distancias 
#Param_Vtan_y  =  5.0
#
#TestSatName   = 'AndXVII' # funciona relativamente bien 
#Param_Vtan_y  =  -260.0
#
TestSatName   = 'AndXXV' # funciona bien! 
#Param_Vtan_y  =  -180.0
#
#TestSatName   = 'NGC147' #funciona! 
#Param_Vtan_y  =  -230.0


#TestSatName   = 'NGC185' #NO funciona! 
#Param_Vtan_y  =  -230.0


#TestSatName   = 'AndIX' 
#Param_Vtan_y  = -130.# NO funciona muy bien que digamos  *******************************************************************************************************************************************************************************************************************************************************

#TestSatName   = 'AndXII' #NO funciona NADA# en un paper http://arxiv.org/pdf/1205.6864v1.pdf dejan a esta por fuera porque asumen que esta en un first infall... ********************************************************************************************************************************************************************************************************************************************************************************************************
#Param_Vtan_y  =   0 #

#TestSatName   = 'AndXIII' #NO funciona # este satelite es el que va para el otro lado # this satellites is counter-rotating ***********************************************************************************************************************************************************************************
#Param_Vtan_y  =  50.0
#
#TestSatName   = 'AndXXVI' #por ahora no funciona mirar nuevamente con mas calma luego... 
#Param_Vtan_y  =  -50.
#
#TestSatName   = 'AndXXVII'  #por ahora no funciona mirar nuevamente con mas calma luego...
#Param_Vtan_y  =  -110.0
#
#TestSatName   = 'AndXXX-CassII' #por ahora no funciona mirar nuevamente con mas calma luego... 
#Param_Vtan_y  =  -70.0

#TestSatName   = 'NGC185'  #por ahora no funciona mirar nuevamente con mas calma luego... 
#Param_Vtan_y  =  -120.0
...
class body(object):
	def __init__( self, Dist, errDistplus , errDistminus , rascenHH , rascenMM , rascenSS , declG , declMM , declSS , Vls , errVlsplus , errVlsminus , m0, name, PlaneYN, Vtany_param ):
		...
		## calculate x, y, z ranges from distance errors and store them
		#old_Xsat_M31max, old_Ysat_M31max, old_Zsat_M31max, old_VXsat_M31, old_VYsat_M31, old_VZsat_M31, eta, xi = getM31centeredCoorndinates(decl, rascen, Dist+errDistplus, Vls)
		#old_Xsat_M31min, old_Ysat_M31min, old_Zsat_M31min, old_VXsat_M31, old_VYsat_M31, old_VZsat_M31, eta, xi = getM31centeredCoorndinates(decl, rascen, Dist+errDistminus, Vls)
		#self.x_max_br = old_Xsat_M31max		
		#self.y_max_br = old_Ysat_M31max
		#self.z_max_br = old_Zsat_M31max
		#self.x_min_br = old_Xsat_M31min
		#self.y_min_br = old_Ysat_M31min
		#self.z_min_br = old_Zsat_M31min
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		posSat = np.array([-self.x,-self.y, -self.z ])
		posSat_modulus = np.sqrt((posSat*posSat).sum())
		planeVec = np.array([-0.158, -0.769, -0.620 ])
		planeVec_modulus = np.sqrt((planeVec*planeVec).sum())
		dotH1 = np.dot(posSat, planeVec)
		cos_angleH1 = dotH1 / posSat_modulus / planeVec_modulus # cosine of angle between poshalo and e3
		angleH1 = - np.arccos(cos_angleH1) + 0.5*np.pi
		angleH1 = angleH1* 180 / np.pi
		#unit_plane_x, unit_plane_y, unit_plane_z =  
		print "angleH1" , angleH1, "Plane", self.PlaneYN, "name", self.name 
		...
		#print " PLANE YN", self.PlaneYN	
		#print " unit_plane_x, 0.158", unit_plane_x	
		#print " unit_plane_y, 0.769", unit_plane_y	
		#print " unit_plane_z, 0.620", unit_plane_z	
		#print " Mag_unit_plane ",  	
		#calculate angle between the two plane vectors:
		#cosThetaplanes  = unit_plane_x*0.158 + unit_plane_y*0.769 + unit_plane_z*0.620
		#theta_planes = np.arccos(cosThetaplanes)*180/np.pi
 		#print " theta_planes1 ", theta_planes1
...
###### xxxxxx 				  
###### xxxxxx 		# 3) Construct tangential velocity
###### xxxxxx 		# conditions: 
###### xxxxxx 		# a) Vtan perpendicular to the line of sight velocity
###### xxxxxx 		# b) Vtot (Vls + Vtan) contained in the plane of satellites
###### xxxxxx 				  
###### xxxxxx 		# Vector perpendicular to the plane of satellites:
###### xxxxxx 		unit_V_pl_x = unit_plane_x #-0.158  
###### xxxxxx 		unit_V_pl_y = unit_plane_y #-0.769 
###### xxxxxx 		unit_V_pl_z = unit_plane_z #-0.620 
###### xxxxxx 
###### xxxxxx 		# step 1: choose one of the components of Vtan
###### xxxxxx 		if self.name == TestSatName: 
###### xxxxxx 			Vtan_y= Param_Vtan_y
###### xxxxxx 		else:	
###### xxxxxx 			Vtan_y= 1.
###### xxxxxx 		# step 2: construct the other 2 components so that Vtan satisfies conditions a) and b)
###### xxxxxx 		auxVtan= self.Vls_x/(unit_V_pl_x*self.Vls_z-self.Vls_x*unit_V_pl_z)     
###### xxxxxx 		auxVtan2=  self.Vls_x*unit_V_pl_x + self.Vls_y*unit_V_pl_y+self.Vls_z*unit_V_pl_z     
###### xxxxxx 		Vtan_z= auxVtan * (Vtan_y*unit_V_pl_y - unit_V_pl_x*self.Vls_y*Vtan_y/self.Vls_x + auxVtan2) 
###### xxxxxx 		Vtan_x=-(Vtan_y*self.Vls_y+Vtan_z*self.Vls_z)/self.Vls_x
###### xxxxxx 
###### xxxxxx 		
###### xxxxxx 		self.Vtan_x = Vtan_x
###### xxxxxx 		self.Vtan_y = Vtan_y
###### xxxxxx 		self.Vtan_z = Vtan_z
###### xxxxxx 		Vtan = np.sqrt(Vtan_x*Vtan_x+Vtan_y*Vtan_y+Vtan_z*Vtan_z)
###### xxxxxx 		self.Vtan = Vtan
###### xxxxxx 
###### xxxxxx 		#Test to see if Vtan satisfies conditions a) and b) 
###### xxxxxx 		Vtan_dot_Vls = self.Vls_x*self.Vtan_x + self.Vls_y*self.Vtan_y + self.Vls_z*self.Vtan_z
###### xxxxxx 		Vtan_dot_Vpl = (Vtan_x+self.Vls_x) * unit_V_pl_x + (Vtan_y+self.Vls_y) * unit_V_pl_y + (Vtan_z+self.Vls_z) * unit_V_pl_z
###### xxxxxx 		print 'Vtan_dot_Vls, Vtan_dot_Vpl' , Vtan_dot_Vls, Vtan_dot_Vpl
###### xxxxxx 
###### xxxxxx 		#M31 velocity after rotation
###### xxxxxx 		VM31x, VM31y, VM31z = rotateZXZ( VM31_x_obs , VM31_y_obs , VM31_z_obs,  zxz_alpha_rz, zxz_beta_rx, zxz_gamma_rz )
###### xxxxxx 		#print 'VM31x, VM31y, VM31z ' , VM31x, VM31y, VM31z	
###### xxxxxx 		
###### xxxxxx 		
###### xxxxxx 		
###### xxxxxx 		#Satellite velocity: line of sight velocity + tangential component - M31 velocity after rotation
###### xxxxxx 		self.Vx = self.Vls_x+ self.Vtan_x - VM31x
###### xxxxxx 		self.Vy = self.Vls_y+ self.Vtan_y - VM31y
###### xxxxxx 		self.Vz = self.Vls_z+ self.Vtan_z - VM31z
###### xxxxxx 		Vel= np.sqrt(self.Vx*self.Vx+self.Vy*self.Vy+self.Vz*self.Vz)
###### xxxxxx 		self.Vel = Vel
###### xxxxxx 
###### xxxxxx 
###### xxxxxx 
###### xxxxxx 
###### xxxxxx 		#test compute the component of the velocity vector in the unit_V_ls direction
###### xxxxxx 		oneoverV_ls = 1/ Vls
###### xxxxxx 		unit_V_ls_x = self.Vls_x*oneoverV_ls 
###### xxxxxx 		unit_V_ls_y = self.Vls_y*oneoverV_ls 
###### xxxxxx 		unit_V_ls_z = self.Vls_z*oneoverV_ls 
###### xxxxxx 		#Vtest = self.Vx*unit_V_ls_x + self.Vy*unit_V_ls_y + self.Vz*unit_V_ls_z 	
###### xxxxxx 		#print "satName, Vls , Vtest ", self.name, self.Vls, Vtest
###### xxxxxx 
###### xxxxxx 		# compute error bars for the initial position and velocity values in the M31galactic coordinate system	
###### xxxxxx 		#self.errR, self.errVel, self.errtanVel = self.ComputeErrorBars (decl, rascen, Dist, errDistplus, errDistminus, Vls, errVlsplus, errVlsminus)
###### xxxxxx 
###### xxxxxx 		# screen print of the Satellite's data (optional)	
###### xxxxxx 		#self.ScreenPrintSatData (rascen, decl, RascenM31, DeclM31, old_Xsat_M31 , old_Ysat_M31 , old_Zsat_M31, old_VXsat_M31, old_VYsat_M31, old_VZsat_M31)
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				  
				  
		VM31x, VM31y, VM31z = rotateZXZ( VM31_x_obs , VM31_y_obs , VM31_z_obs,  zxz_alpha_rz, zxz_beta_rx, zxz_gamma_rz )
		#VM31x, VM31y, VM31z = 0. , 0. , 0.
		...
		# Vector perpendicular to the plane of satellites:
		#unit_V_pl_x = -0.158  
		#unit_V_pl_y = -0.769 
		#unit_V_pl_z = -0.620 
		...
		#auxVtan= self.Vls_x/(self.Vls_z*unit_V_pl_x+self.Vls_x*unit_V_pl_z)
		#auxVtan2= self.Vls_x*unit_V_pl_x + self.Vls_y*unit_V_pl_y + self.Vls_z*unit_V_pl_z - VM31x*unit_V_pl_x - VM31y*unit_V_pl_y - VM31z*unit_V_pl_z -  self.Vls_y*Vtan_y*unit_V_pl_x/self.Vls_x + Vtan_y*unit_plane_y
		
		Vtan_z = auxVtan * auxVtan2		
		
		Vtan_x= -((self.Vls_y*Vtan_y + self.Vls_z*Vtan_z)/self.Vls_x)

		#self.Vls_x/(unit_V_pl_x*self.Vls_z-self.Vls_x*unit_V_pl_z)     
		#auxVtan2=  self.Vls_x*unit_V_pl_x + self.Vls_y*unit_V_pl_y+self.Vls_z*unit_V_pl_z     
		#Vtan_z= auxVtan * (Vtan_y*unit_V_pl_y - unit_V_pl_x*self.Vls_y*Vtan_y/self.Vls_x + auxVtan2) 
		#Vtan_x=-(Vtan_y*self.Vls_y+Vtan_z*self.Vls_z)/self.Vls_x
		...
		#Test to see if Vtan satisfies conditions a) and b) 
		Vtan_dot_Vls = self.Vls_x*self.Vtan_x + self.Vls_y*self.Vtan_y + self.Vls_z*self.Vtan_z
		#Vtan_dot_Vpl = (Vtan_x+self.Vls_x) * unit_V_pl_x + (Vtan_y+self.Vls_y) * unit_V_pl_y + (Vtan_z+self.Vls_z) * unit_V_pl_z
		Vtan_dot_Vpl2 = (Vtan_x+self.Vls_x- VM31x) * unit_V_pl_x + (Vtan_y+self.Vls_y- VM31y) * unit_V_pl_y + (Vtan_z+self.Vls_z- VM31z) * unit_V_pl_z
		#print 'Vtan_dot_Vls, Vtan_dot_Vpl, Vtan_dot_Vpl2' , Vtan_dot_Vls, Vtan_dot_Vpl, Vtan_dot_Vpl2
		print 'Vtan_dot_Vls, Vtan_dot_Vpl2' , Vtan_dot_Vls, Vtan_dot_Vpl2

		#M31 velocity after rotation
		VM31x, VM31y, VM31z = rotateZXZ( VM31_x_obs , VM31_y_obs , VM31_z_obs,  zxz_alpha_rz, zxz_beta_rx, zxz_gamma_rz )
		#print 'VM31x, VM31y, VM31z ' , VM31x, VM31y, VM31z	
		...
		#Vtest = self.Vx*unit_V_ls_x + self.Vy*unit_V_ls_y + self.Vz*unit_V_ls_z 	
		#print "satName, Vls , Vtest ", self.name, self.Vls, Vtest

		# compute error bars for the initial position and velocity values in the M31galactic coordinate system	
		#self.errR, self.errVel, self.errtanVel = self.ComputeErrorBars (decl, rascen, Dist, errDistplus, errDistminus, Vls, errVlsplus, errVlsminus)

		# screen print of the Satellite's data (optional)	
		#self.ScreenPrintSatData (rascen, decl, RascenM31, DeclM31, old_Xsat_M31 , old_Ysat_M31 , old_Zsat_M31, old_VXsat_M31, old_VYsat_M31, old_VZsat_M31)

		self.StoreSpatialCoordsInit ()
		#self.ComputeAngularMomentumIni ()
	...
	def compute_potentialENER( self ):
		...
		devidewith=(rbulge+rsqt)    #V *(rbulge+rsqt)*rsqt
		...
		DiskConst=-2.0*np.pi*disksurface*rdisk*rdisk*(1.- np.power(e,-rsqt/rdisk))/rsqt
		HaloConst=-(Mhalo/rsqt)*np.log((rsqt/rhalo)+1)      #4.0*np.pi*halodensity*criticaldensity*(rhalo*rhalo*rhalo)*(1./(rsqt*rsqt))*(1./(rsqt+rhalo)-(np.log(rsqt/rhalo+1.)/rsqt))
	
		FixPotx = G*(HaloConst+DiskConst+BulgeConst)#*self.x 
		FixPoty = G*(HaloConst+DiskConst+BulgeConst)#*self.y 
		FixPotz = G*(HaloConst+DiskConst+BulgeConst)#*self.z 
		Epotx = FixPotx
		Epoty = FixPoty
		Epotz = FixPotz
		#print self.name,": Fx Fy Fz pot", Fx/self.m, Fy/self.m, Fz/self.m 
		return Epotx, Epoty, Epotz
...
def getM31centeredCoorndinates(decl, rascen, Dist, Vls):	
	...
	# Vybrot300_ini = Vls*np.sin(eta)
	# Vxbrot300_ini = Vls*costheta*np.tan(xi)
	# print "antes", old_VXsat_M31, old_VYsat_M31 , old_VZsat_M31
	# print "ahora", testVx, testVy, testVz
...
def plotDistAverPlane ():
	#plotting the satellite-plane average distance and its dispersion 
	plt.figure()
	plt.ylim( -1., 250. )
	#plt.plot(data_DistAver_PlaneSat, label= 'Average distance (PlaneSat)')
	plt.plot(data_realTime, data_DistAver_PlaneSat, color = "red")
	plt.plot(data_realTime, data_DistAver_plusDisp_PlaneSat, color = "red")
	plt.plot(data_realTime, data_DistAver_minusDisp_PlaneSat, color = "red")
	#plt.legend()
	plt.xlabel('Time (Gyr)', fontsize=15) 
	plt.ylabel('Average distance Satellites-Plane (kpc)', fontsize=15)
	#plt.savefig('DistAver_PlaneSat_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+VelType+'.eps')
	
	plt.figure()
	plt.ylim( -1., 250. )
	#plt.plot(data_DistAver_NotPlaneSat, label= 'Average distance (NotPlaneSat)')
	plt.plot(data_realTime, data_DistAver_NotPlaneSat, color= 'blue')
	plt.plot(data_realTime, data_DistAver_plusDisp_NotPlaneSat, color= 'blue')
	plt.plot(data_realTime, data_DistAver_minusDisp_NotPlaneSat, color= 'blue')
	#plt.legend()
	plt.xlabel('Time (Gyr)', fontsize=15) 
	plt.ylabel('Average distance Satellites-Plane (kpc)', fontsize=15)
	#plt.savefig('DistAver_NotPlaneSat_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+VelType+'.eps')
	
	plt.figure()
	plt.ylim( -1., 250. )
	#plt.plot(data_DistAver_All, label= 'Average distance (All)')
	plt.plot(data_realTime, data_DistAver_All)
	plt.plot(data_realTime, data_DistAver_plusDisp_All)
	plt.plot(data_realTime, data_DistAver_minusDisp_All)
	#plt.legend()
	plt.xlabel('Time (Gyr)', fontsize=15) 
	plt.ylabel('Average distance Satellites-Plane (kpc)', fontsize=15)
	#plt.savefig('DistAver_All_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+VelType+'.eps')
	#plt.show()

	#plotting the satellite-plane average distance and its dispersion 
	plt.figure()
	plt.ylim( -1., 250. )
	plt.plot(data_realTime, data_DistAver_PlaneSat, color = "red", linewidth=4, label= "Plane Satellites")
	plt.plot(data_realTime, data_DistAver_plusDisp_PlaneSat, color = "red")
	plt.plot(data_realTime, data_DistAver_minusDisp_PlaneSat, color = "red")
	plt.plot(data_realTime, data_DistAver_NotPlaneSat, color= 'blue', linewidth=4, label= "Other Satellites")
	plt.plot(data_realTime, data_DistAver_plusDisp_NotPlaneSat,  color= 'blue')
	plt.plot(data_realTime, data_DistAver_minusDisp_NotPlaneSat, color= 'blue')
	plt.legend()
	plt.xlabel('Time (Gyr)', fontsize=15) 
	plt.ylabel('Average distance Satellites-Plane (kpc)', fontsize=15)
	#plt.savefig('DistAver_PlaneNotPlane_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+VelType+'.eps')
	plt.savefig('DistAver_PlaneNotPlane_'+NumIntegr+'_'+SatSatInter+'_'+MW_POT+'.pdf')

def plotDistPlane ( ):
	#plotting the satellite-plane individual distances 
	plt.figure()
	for sat in Satellite:
		#plt.ylim( -100., 100. )
		plt.ylim( -150., 150. )
		if sat.PlaneYN == "Yes":
			plt.plot(data_realTime, sat.data_distPlane, color ="red")
		if sat.PlaneYN == "No":
			plt.plot(data_realTime , sat.data_distPlane, color = "blue")
	plt.xlabel('Time (Gyr)', fontsize=15) 
	plt.ylabel('Distance Satellite-Plane (kpc)', fontsize=15)
	#plt.title('titulo')
	#plt.savefig('DistPlane_All_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+VelType+'.eps')
	plt.savefig('DistPlane_All_'+NumIntegr+'_'+SatSatInter+'_'+MW_POT+'.pdf')
	...
	for sat in Satellite:
		...
		plt.ylim( -150., 150. )
		if sat.PlaneYN == "Yes":
			plt.plot(sat.data_distPlane, color ="red")
	plt.xlabel('Time (Gyr)', fontsize=15) 
	plt.ylabel('Distance Satellite-Plane (kpc)', fontsize=15)
	#plt.savefig('DistPlane_PlaneSat_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+VelType+'.eps')
	...
	for sat in Satellite:
		...
		plt.ylim( -150., 150. )
		if sat.PlaneYN == "No":
			plt.plot(sat.data_distPlane, color = "blue")
	plt.xlabel('Time (Gyr)', fontsize=15) 
	plt.ylabel('Distance Satellite-Plane (kpc)', fontsize=15)
	#plt.savefig('DistPlane_NotPlaneSat_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+VelType+'.eps')
		
def plotInitialVelocities (SatList):
	...
	for sat in Satellite:
	       	ax = plt.gca()
		ax.quiver(sat.x, sat.y, sat.Vx, sat.Vy ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		ax.quiver(sat.x, sat.y, sat.Vls_x, sat.Vls_y ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		ax.quiver(sat.x, sat.y, sat.Vtan_x, sat.Vtan_y ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Y (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_xy_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'.pdf')
	...
	for sat in Satellite:
	       	ax = plt.gca()
		ax.quiver(sat.x, sat.z, sat.Vx, sat.Vz ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		ax.quiver(sat.x, sat.z, sat.Vls_x, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		ax.quiver(sat.x, sat.z, sat.Vtan_x, sat.Vtan_z ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_xz_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'.pdf')
	...
	for sat in Satellite:
	       	ax = plt.gca()
		ax.quiver(sat.y, sat.z, sat.Vy, sat.Vz ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		ax.quiver(sat.y, sat.z, sat.Vls_y, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		ax.quiver(sat.y, sat.z, sat.Vtan_y, sat.Vtan_z ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
	plt.xlabel('Y (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_yz_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'.pdf')

def plotInitialVelocities_WITHOUTsubs_VM31 (SatList, dM31obs, vM31x_obs, vM31y_obs, vM31z_obs):
	...
	#M31 velocity after rotation
	VM31x, VM31y, VM31z = rotateZXZ( vM31x_obs , vM31y_obs , vM31z_obs,  zxz_alpha_rz, zxz_beta_rx, zxz_gamma_rz )
	...
	for sat in Satellite:
	       	ax = plt.gca()
		ax.quiver(sat.x, sat.y, (sat.Vx+VM31x), (sat.Vy+VM31y) ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		ax.quiver(sat.x, sat.y, sat.Vls_x, sat.Vls_y ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		ax.quiver(sat.x, sat.y, sat.Vtan_x, sat.Vtan_y ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Y (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_xy_WM31'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'.pdf')
	...
	for sat in Satellite:
	       	ax = plt.gca()
		ax.quiver(sat.x, sat.z, (sat.Vx+VM31x), (sat.Vz+VM31z) ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		ax.quiver(sat.x, sat.z, sat.Vls_x, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		ax.quiver(sat.x, sat.z, sat.Vtan_x, sat.Vtan_z ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_xz_WM31'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'.pdf')
	...
	for sat in Satellite:
	       	ax = plt.gca()
		ax.quiver(sat.y, sat.z, (sat.Vy+VM31y), (sat.Vz+VM31z) ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		ax.quiver(sat.y, sat.z, sat.Vls_y, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		ax.quiver(sat.y, sat.z, sat.Vtan_y, sat.Vtan_z ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
	plt.xlabel('Y (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_yz_WM31'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'.pdf')



def plotSnapshotsAll (SatList, tSnap, dM31obs, vM31x_obs, vM31y_obs, vM31z_obs):
	...
	for sat in SatList:
		if sat.PlaneYN == "yes":
			oneover_ls2= 1/( (sat.x-x_MW0)*(sat.x-x_MW0)+ (sat.y-y_MW0)*(sat.y-y_MW0) + (sat.z-z_MW0)*(sat.z-z_MW0) )
			auxProj = (sat.x-x_MW0)*(sat.Vx+VM31x)+ (sat.y-y_MW0)*(sat.Vy+VM31y) + (sat.z-z_MW0)*(sat.Vz+VM31z)
			ProjX= auxProj*(sat.x-x_MW0) *oneover_ls2
			ProjY= auxProj*(sat.y-y_MW0) *oneover_ls2
			ProjZ= auxProj*(sat.z-z_MW0) *oneover_ls2
		
			#line of sight velocity (for MW initial position)
			Vls_Xpro = ProjX
			Vls_Ypro = ProjY
			Vls_Zpro = ProjZ
	...
	plt.ylim( -500., 500. )
	ax = plt.gca()
	circle1 = Circle( (x_MW0,y_MW0), 3.5, facecolor='none', edgecolor='k', linewidth=2 )
	circle2 = Circle( (0.,0.), 3.5, facecolor='none', edgecolor='k', linewidth=2 )
	...
	for sat in Satellite:
	        ...
	        #	ax = plt.gca()
		#	plt.plot(sat.data_x, sat.data_y, color="grey", linewidth=0.5)
		#	plt.plot(sat.x, sat.y, '+', markersize=10, color= "red")
		#	ax.quiver(sat.x, sat.y, sat.Vx, sat.Vy ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		#	ax.quiver(sat.x, sat.y, Vls_Xpro, Vls_Ypro ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		#	ax.quiver(sat.x, sat.y, sat.Vtan_x, sat.Vtan_y ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
		if sat.PlaneYN == "Yes":
	        	ax = plt.gca()
			plt.plot(sat.data_x, sat.data_y, color="grey", linewidth=0.5)
			plt.plot(sat.data_x[0], sat.data_y[0], '.', markersize=10, color= "red")
			ax.quiver(sat.data_x[0], sat.data_y[0], sat.Vls_x, sat.Vls_y ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "YesNO":
			...
			ax.quiver(sat.data_x[0], sat.data_y[0], sat.Vls_x, sat.Vls_y ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "No":
			plt.plot(sat.data_x[0], sat.data_y[0], '.', markersize=10, color= "blue")
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Y (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_xy_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
	...
	plt.ylim( -500., 500. )
	ax = plt.gca()
	circle1 = Circle( (x_MW0,z_MW0), 3.5, facecolor='none', edgecolor='k', linewidth=2 )
	circle2 = Circle( (0.,0.), 3.5, facecolor='none', edgecolor='k', linewidth=2 )
	ax.add_artist(circle1)
	ax.add_artist(circle2)
	for sat in Satellite:
		#if sat.name == TestSatName:
		#	ax = plt.gca()
		#	plt.plot(sat.data_x, sat.data_z, color="grey", linewidth=0.5)
		#	plt.plot(sat.x, sat.z, '*', markersize=10, color= "red")
		#	ax.quiver(sat.x, sat.z, sat.Vx, sat.Vz ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.5)
		#	ax.quiver(sat.x, sat.z, Vls_Xpro, Vls_Zpro ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		#	ax.quiver(sat.x, sat.z, sat.Vtan_x, sat.Vtan_z ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
		if sat.PlaneYN == "Yes":
			ax = plt.gca()
			plt.plot(sat.data_x, sat.data_z, color="grey", linewidth=0.5)
			plt.plot(sat.data_x[0], sat.data_z[0], '.', markersize=10, color= "red")
			ax.quiver(sat.data_x[0], sat.data_z[0], sat.Vls_x, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "YesNO":
			ax = plt.gca()
			#plt.plot(sat.data_x, sat.data_z, color="grey", linewidth=0.5)
			plt.plot(sat.data_x[0], sat.data_z[0], '*', markersize=10, color= "red")
			ax.quiver(sat.data_x[0], sat.data_z[0], sat.Vls_x, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "No":
			plt.plot(sat.data_x[0], sat.data_z[0], '.', markersize=10, color= "blue")
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_xz_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
	...
	plt.ylim( -500., 500. )
	ax = plt.gca()
	circle1 = Circle( (y_MW0,z_MW0), 3.5, facecolor='none', edgecolor='k', linewidth=2 )
	circle2 = Circle( (0.,0.), 3.5, facecolor='none', edgecolor='k', linewidth=2 )
	ax.add_artist(circle1)
	ax.add_artist(circle2)
	for sat in Satellite:
		#if sat.name == TestSatName:
		#	ax = plt.gca()
		#	plt.plot(sat.data_y, sat.data_z, color="grey", linewidth=0.5)
		#	plt.plot(sat.y, sat.z, '*', markersize=10, color= "red")
		#	ax.quiver(sat.y, sat.z, sat.Vy, sat.Vz ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.5)
		#	ax.quiver(sat.y, sat.z, Vls_Ypro, Vls_Zpro ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		#	ax.quiver(sat.y, sat.z, sat.Vtan_y, sat.Vtan_z ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
		if sat.PlaneYN == "Yes":
			ax = plt.gca()
			plt.plot(sat.data_y, sat.data_z, color="grey", linewidth=0.5)
			plt.plot(sat.data_y[0], sat.data_z[0], '.', markersize=10, color= "red")
			ax.quiver(sat.data_y[0], sat.data_z[0], sat.Vls_y, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "YesNO":
			ax = plt.gca()
			#plt.plot(sat.data_y, sat.data_z, color="grey", linewidth=0.5)
			plt.plot(sat.data_y[0], sat.data_z[0], '*', markersize=10, color= "red")
			ax.quiver(sat.data_y[0], sat.data_z[0], sat.Vls_y, sat.Vls_z ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "No":
			plt.plot(sat.data_y[0], sat.data_z[0], '.', markersize=10, color= "blue")
	plt.xlabel('Y (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_yz_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')


def plotSnapshotsAll_beforeRotation (SatList, tSnap, dM31obs, vM31x_obs, vM31y_obs, vM31z_obs):
	...
	#for sat in SatList:
	#	if sat.name == TestSatName:
	#		oneover_ls2= 1/( (sat.xbr-old_x_MW)*(sat.xbr-old_x_MW)+ (sat.ybr-old_y_MW)*(sat.ybr-old_y_MW) + (sat.zbr-old_z_MW)*(sat.zbr-old_z_MW) )
	#		auxProj = (sat.xbr-old_x_MW)*(sat.Vxbr+vM31x_obs)+ (sat.ybr-old_y_MW)*(sat.Vybr+vM31y_obs) + (sat.zbr-old_z_MW)*(sat.Vzbr+vM31z_obs)
	#		ProjX= auxProj*(sat.xbr-old_x_MW) *oneover_ls2
	#		ProjY= auxProj*(sat.ybr-old_y_MW) *oneover_ls2
	#		ProjZ= auxProj*(sat.zbr-old_z_MW) *oneover_ls2
	#	
	#		#line of sight velocity (for MW initial position)
	#		Vls_Xpro_br = ProjX
	#		Vls_Ypro_br = ProjY
	...
	for sat in Satellite:
		...
		#	plt.plot(sat.xbr, sat.ybr, '+', markersize=10, color= "red")
		#	ax.quiver(sat.xbr, sat.ybr, sat.Vxbr, sat.Vybr ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		#	ax.quiver(sat.xbr, sat.ybr, Vls_Xpro_br, Vls_Ypro_br ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		#	ax.quiver(sat.xbr, sat.ybr, Vtan_xbr, Vtan_ybr ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
		if sat.PlaneYN == "Yes":
	        	ax = plt.gca()
			plt.plot(sat.data_xbr, sat.data_ybr, color="grey", linewidth=0.5)
	...
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Y (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_BeforeRotation_xy_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
	...
	for sat in Satellite:
		...
		#	plt.plot(sat.xbr, sat.zbr, '+', markersize=10, color= "red")
		#	ax.quiver(sat.xbr, sat.zbr, sat.Vxbr, sat.Vzbr ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		#	ax.quiver(sat.xbr, sat.zbr, Vls_Xpro_br, Vls_Zpro_br ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		#	ax.quiver(sat.xbr, sat.zbr, Vtan_xbr, Vtan_zbr ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
		if sat.PlaneYN == "Yes":
	        	ax = plt.gca()
			plt.plot(sat.data_xbr, sat.data_zbr, color="grey", linewidth=0.5)
			plt.plot(sat.data_xbr[0], sat.data_zbr[0], '.', markersize=10, color= "red")
			ax.quiver(sat.data_xbr[0], sat.data_zbr[0], Vls_xbr, Vls_zbr ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "YesNO":
	        	ax = plt.gca()
			#plt.plot(sat.data_xbr, sat.data_zbr, color="grey", linewidth=0.5)
			plt.plot(sat.data_xbr[0], sat.data_zbr[0], '*', markersize=10, color= "red")
	...
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_BeforeRotation_xz_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
	...
	for sat in Satellite:
		...
		#	plt.plot(sat.ybr, sat.zbr, '+', markersize=10, color= "red")
		#	ax.quiver(sat.ybr, sat.zbr, sat.Vybr, sat.Vzbr ,angles='xy',scale_units='xy',scale=1, color= 'grey' , linewidth=0.25)
		#	ax.quiver(sat.ybr, sat.zbr, Vls_Ypro_br, Vls_Zpro_br ,angles='xy',scale_units='xy',scale=1, color= 'black' , linewidth=0.25)
		#	ax.quiver(sat.ybr, sat.zbr, Vtan_ybr, Vtan_zbr ,angles='xy',scale_units='xy',scale=1, color= 'green' , linewidth=0.125)
		if sat.PlaneYN == "Yes":
	        	ax = plt.gca()
			plt.plot(sat.data_ybr, sat.data_zbr, color="grey", linewidth=0.5)
			plt.plot(sat.data_ybr[0], sat.data_zbr[0], '.', markersize=10, color= "red")
			ax.quiver(sat.data_ybr[0], sat.data_zbr[0], Vls_ybr, Vls_zbr ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "YesNO":
	        	ax = plt.gca()
			#plt.plot(sat.data_ybr, sat.data_zbr, color="grey", linewidth=0.5)
			plt.plot(sat.data_ybr[0], sat.data_zbr[0], '*', markersize=10, color= "red")
	...
	plt.xlabel('Y (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_BeforeRotation_yz_'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
...
def plotSnapshotsAll_beforeRotation_VelMin300 (SatList, tSnap, dM31obs, vM31x_obs, vM31y_obs, vM31z_obs):
	...
	for sat in Satellite:
		...
		if sat.PlaneYN == "Yes":
	        	ax = plt.gca()
			#plt.plot(sat.data_xbr, sat.data_ybr, color="grey", linewidth=1.)
			if sat.name == TestSatName: 
				plt.plot(sat.data_xbr, sat.data_ybr, color="black", linewidth=1.5)
			plt.plot(sat.data_xbr[0], sat.data_ybr[0], '.', markersize=15, color= "red")
			#ax.quiver(sat.data_xbr[0], sat.data_ybr[0], Vls_xbr, Vls_ybr ,angles='xy',scale_units='xy',scale=1, color= 'red' , linewidth=0.15)
		if sat.PlaneYN == "YesNO":
	        	ax = plt.gca()
			#plt.plot(sat.data_xbr, sat.data_ybr, color="grey", linewidth=0.5)
	...
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Y (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_xy_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_xy_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.png')
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_xy_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.jpg')
	...
	for sat in Satellite:
		...
		if sat.PlaneYN == "Yes":
	        	ax = plt.gca()
			#plt.plot(sat.data_xbr, sat.data_zbr, color="grey", linewidth=1.)
			if sat.name == TestSatName: 
				plt.plot(sat.data_xbr, sat.data_zbr, color="black", linewidth=1.5)
			plt.plot(sat.data_xbr[0], sat.data_zbr[0], '.', markersize=15, color= "red")
	...
	plt.xlabel('X (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_xz_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_xz_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.png')
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_xz_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.jpg')
	...
	for sat in Satellite:
		...
		if sat.PlaneYN == "Yes":
	        	ax = plt.gca()
			#plt.plot(sat.data_ybr, sat.data_zbr, color="grey", linewidth=1.)
			if sat.name == TestSatName: 
				plt.plot(sat.data_ybr, sat.data_zbr, color="black", linewidth=1.5)
			plt.plot(sat.data_ybr[0], sat.data_zbr[0], '.', markersize=15, color= "red")
	...
	plt.xlabel('Y (kpc)', fontsize=15) 
	plt.ylabel('Z (kpc)', fontsize=15)
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_yz_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.pdf')
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_yz_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.png')
	plt.savefig('SnapshotSatellites_All_BeforeRotationM300_yz_soloTESTsat'+TestSatName+'_'+NumIntegr+'_'+SatSatInter+MW_POT+'_'+tSnap+'.jpg')
...
def TestLineOfSightVelocity(SatList, dM31obs, vM31x_obs, vM31y_obs, vM31z_obs):
	#calculate initial MW position in the M31-centric coordinate system
	old_x_MW = 0.
	old_y_MW = 0.	
	old_z_MW = - dM31obs
	...
	for sat in SatList:
		...
		Vls_Yini = ProjY
		Vls_Zini = ProjZ
		print "name, Vls, VlsProjection", sat.name, sat.Vls_x, sat.Vls_y, sat.Vls_z
		print "name, Vls, VlsProjection", sat.name, Vls_Xini, Vls_Yini, Vls_Zini
...
#TEST plot r vs angular momentum
posHalo=[]
L_halo=[]
...
#plt.ylim( -0.01, 50. )
for sat in Satellite:
	posHalo = np.array([0.001*sat.x, 0.001*sat.y, 0.001*sat.z ])
	#print posHalo
	posHalo_modulus = np.sqrt((posHalo*posHalo).sum())
	velHalo = np.array([sat.Vx, sat.Vy, sat.Vz ])
	#print velHalo
	L_halo = np.cross(posHalo, velHalo)	
	print "Lhalo", L_halo	
	L_halo_z = L_halo[2]	
	print "LhaloZ", L_halo_z 	
	L_halo_z_modulus = np.sqrt(L_halo_z*L_halo_z)	
	L_halo_modulus = np.sqrt((L_halo*L_halo).sum())
	
	vel_Halo_modulus = np.sqrt((velHalo*velHalo).sum())
        Ekin = 0.5*vel_Halo_modulus*vel_Halo_modulus
        Epot , Epot, Epot = sat.compute_potentialENER()
        #Epot = 0.5*vel_Halo_modulus*vel_Halo_modulus
        #Etot = 0.5*vel_Halo_modulus*vel_Halo_modulus
	#posHalo.append(posHalo_modulus)
	#L_halo.append(L_halo_modulus)
	#print posHalo_modulus
	#print L_halo_modulus
	if sat.PlaneYN == "Yes":
		#print posHalo_modulus
		print L_halo_modulus
		print L_halo_z_modulus
                #plt.plot(posHalo_modulus,L_halo_modulus,'.', markersize=5,color="red")
                #plt.plot(posHalo_modulus,vel_Halo_modulus,'.', markersize=5,color="red")
                #plt.plot(vel_Halo_modulus,L_halo_modulus,'.', markersize=5,color="red")
                plt.plot(Ekin+Epot,L_halo_z_modulus,'.', markersize=15,color="red")
	if sat.PlaneYN == "YesNO":
		#print posHalo_modulus
		print L_halo_modulus
		print L_halo_z_modulus
                ...
                plt.plot(Ekin+Epot,L_halo_z_modulus,'*', markersize=15,color="red")
	if sat.PlaneYN == "No":
                #plt.plot(posHalo_modulus,L_halo_modulus,'.', markersize=5,color="blue")
                #plt.plot(posHalo_modulus,vel_Halo_modulus,'.', markersize=5,color="blue")
                #plt.plot(vel_Halo_modulus,L_halo_modulus,'.', markersize=5,color="blue")
                plt.plot(Ekin+Epot,L_halo_z_modulus,'.', markersize=10,color="blue")
#plt.xlabel('Distance to parent halo (Mpc)', fontsize=15) 
plt.xlabel('Total Energy per unit mass', fontsize=15) 
plt.ylabel('Angular momentum per unit mass (Mpc.km/s)', fontsize=15)
plt.savefig('Halo_pos_L.pdf')
...
#INITIAL plots and TESTS

#plotInitialVelocities(SatList= Satellite)

#plotInitialVelocities_WITHOUTsubs_VM31(SatList= Satellite, dM31obs = DM31_obs, vM31x_obs = VM31_x_obs, vM31y_obs = VM31_y_obs, vM31z_obs = VM31_z_obs)

#TestLineOfSightVelocity(SatList= Satellite, dM31obs = DM31_obs, vM31x_obs = VM31_x_obs, vM31y_obs = VM31_y_obs, vM31z_obs = VM31_z_obs)
...
# mirar bien si la velocidad es 300 o no 
old_x_MW = 0.
old_y_MW = 0.	
old_z_MW = - DM31_obs
...
#print "old_z_MW", old_z_MW
#print "old_Vx_MW, old_Vy_MW, old_Vz_MW", old_Vx_MW, old_Vy_MW, old_Vz_MW
...
#print "x_MW, y_MW, z_MW", x_MW0, y_MW0, z_MW0
#print "Vx_MW, Vy_MW, Vz_MW", Vx_MW0, Vy_MW0, Vz_MW0
#print "dist_MW", np.sqrt(x_MW0*x_MW0+y_MW0*y_MW0+z_MW0*z_MW0)
...
##  vv  #plotSnapshotsDistVel(tSnap)
##  vv  plotSnapshotsAll(tSnap,x_MW0, y_MW0, z_MW0, x_MW, y_MW, z_MW)	
##  vv  plotSnapshotsAll_BeforeRotation(tSnap,x_MW0, y_MW0, z_MW0, x_MW, y_MW, z_MW)	
##  vv  plotSnapshotsAll_BeforeRotation300(tSnap,x_MW0, y_MW0, z_MW0, x_MW, y_MW, z_MW)	
##  vv  #plotSnapshotsAll(tSnap,x_MW0, y_MW0, z_MW0)	
##  vv  #plotSnapshotsAllCircles(tSnap)	

#numerical integration

for t in xrange(int(T/DeltaT)):

	#calculate new (old if backwards) position of the MW
	...
	if cont == 200 :
		...
		if t < 100:
			tSnap = "00"+str(t)
	#	plotSnapshotsDistVel(tSnap)
	#	plotSnapshotsAll(Satellite, tSnap, DM31_obs, VM31_x_obs, VM31_y_obs, VM31_z_obs)	
	#	plotSnapshotsAll_beforeRotation (Satellite, tSnap, DM31_obs, VM31_x_obs, VM31_y_obs, VM31_z_obs)
	#	plotSnapshotsAll_beforeRotation_VelMin300 (Satellite, tSnap, DM31_obs, VM31_x_obs, VM31_y_obs, VM31_z_obs)
		#plotSnapshotsAll_BeforeRotation(tSnap,x_MW0, y_MW0, z_MW0, x_MW, y_MW, z_MW)	
		#plotSnapshotsAll_BeforeRotation300(tSnap,x_MW0, y_MW0, z_MW0, x_MW, y_MW, z_MW)
		cont = 0
	cont=cont+1
##plots orbits, Satellite-plane distances and average distance (with its dispersion)	
#*#for sat in Satellite:
#*#	#sat.plotOrbits()
#*#	sat.print_data()
#*#	sat.print_databrot()
#*#	#sat.plotDistVelorbit()
#*#plotMWpositionInM31coords_BeforeRot( data_oldMWx, data_oldMWy,data_oldMWz)
#*#plot_initial_VlsVec_beforeRot ()
#*if Veltan == 'yes':
#*	plot_initial_VlsVec_VelTan ()
#*#plotDistVel_all ()
#*#plotDistVel_PlaneSat ()
#*#plotDistVel_NotPlaneSat ()
#*plotOrbitsDistVel ()
#*#plotOrbits3D ()
#*#if MW_POT == "MW" :
#*#	plotOrbitMW3D ()
...
#for sat in Satellite:
#	if sat.PlaneYN == 'Yes':
#		print "Sat", sat.name, " x=", sat.x0 , " y=" , sat.y0, " z=" , sat.z0

plotDistAverPlane ()
plotDistPlane ()
#for sat in Satellite:
#	sat.print_data(data_realTime)
